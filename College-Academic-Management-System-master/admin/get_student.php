...
if(!empty($_POST["Branchid"])) 
{
 ...
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {?>
  <p> <?php echo htmlentities($row['StudentName']); ?><input type="text"  name="marks[]" value="" class="form-control" required="" placeholder="Enter marks out of 100" autocomplete="off"></p>
...
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT SubjectId,BranchId FROM tblresult WHERE SubjectId=:id1 and BranchId=:id ");
